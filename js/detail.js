...
"use strict"
        ...
        const card = `
            ...
            <aside class="sticky top-6">
                <div class="bg-white rounded-xl shadow p-6 lg:p-8">
                    <h1 class="text-2xl lg:text-3xl font-extrabold leading-tight">${data?.title}<span
                            class="text-base font-medium text-gray-500">• 1.00 kg</span></h1>

                    <div class="mt-4 flex items-end gap-4">
                        <div>
                            <div class="text-3xl lg:text-4xl font-extrabold price-shadow">₹${data?.price}</div>
                            <div class="text-sm text-gray-400 line-through">₹${data?.price}</div>
                ...
                <div id="mobileCta" class="fixed left-0 right-0 bottom-4 px-4 lg:hidden">
                    <div class="max-w-3xl mx-auto">
                        <button id="mobileAdd"
                            class="w-full bg-green-600 text-white rounded-full py-3 font-semibold shadow-lg">🛒 Add
                            to Cart — ₹450</button>
                    </div>
                </div>
    ...
    const proId = searchParam.get('id');
